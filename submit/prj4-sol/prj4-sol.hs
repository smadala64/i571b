...
-- Restriction: MUST use recursions
deptEmployees :: [Employee] -> Department -> [Employee]
deptEmployees [] _ = []
deptEmployees (q:qs) dept
  | emplDept q == dept = q:deptEmployees qs dept
  | otherwise = deptEmployees qs dept
...
comprDeptEmployees q dept  = [r | r <- q, emplDept r == dept] 
...
expnCalc x int char f = v:expnCalc x newint newchar newf
                        where
                            v = int/char
                            newint = int * x
                            newchar = char * f
...
foldTree treeFn leafFn tree = case tree of 
    Leaf m -> leafFn m
    Tree s m q -> treeFn (foldTree treeFn leafFn s) m (foldTree treeFn leafFn q)
...
flattenTree t = foldTree (\s v q -> s ++ [v] ++ q) (\v -> [v]) t
...
catenateTreeLists (t) =  [ m | s <- flattenTree t, m <- s]
